...
RendererB::RendererB(const Swapchain &swapchain, Render::CalcType cType, Render::LineType lType, Render::RecordType rType) : m_cType(cType), m_lType(lType), m_rType(rType)
{
	...
	m_tArray = std::make_unique<TrackArrayB>(*m_linePipeline.get(), cType, lType, rType);
...
vk::CommandBuffer RendererB::draw(unsigned long duration, uint32_t imageIndex)
{
...
void RendererB::drawStatic(uint32_t imageIndex)
{
...
vk::CommandBuffer RendererB::drawDynamic(uint32_t imageIndex, unsigned long duration, glm::vec3 &cameraPos, glm::mat4 &VP)
{
	std::array<vk::ClearValue, 2> clearValues{};
	clearValues[0] = vk::ClearColorValue(std::array<float, 4>({ 0.1f, 0.1f, 0.2f, 1.0f }));
	clearValues[1] = vk::ClearDepthStencilValue(1.0f, 0);
...
void RendererB::initSwapchainDependent(const Swapchain &swapchain)
{
...
void RendererB::freeSwapchainDependent()
{
