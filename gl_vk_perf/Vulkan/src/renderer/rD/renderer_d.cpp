...
RendererD::RendererD(const Swapchain &swapchain, Render::CalcType cType, Render::LineType lType, Render::RecordType rType) : m_cType(cType), m_lType(lType), m_rType(rType)
{
	m_lineVertexShader = Pipeline::createShaderModule("drawid_col.vert.spv");
	...
	m_linePipeline = std::make_unique<LinePipeline>(true);
	m_linePipeline->init();

	m_modelPipeline = std::make_unique<ModelPipeline>();
	m_modelPipeline->init();

	m_tArray = std::make_unique<TrackArrayD>(*m_linePipeline.get(), cType, lType, rType);
...
vk::CommandBuffer RendererD::draw(unsigned long duration, uint32_t imageIndex)
{
...
void RendererD::drawStatic(uint32_t imageIndex)
{
	...
	m_tArray->drawStatic(cmd, m_linePipeline->getPipelineLayout());

	cmd.endRenderPass();
	cmd.end();
}

vk::CommandBuffer RendererD::drawDynamic(uint32_t imageIndex, unsigned long duration, glm::vec3 &cameraPos, glm::mat4 &VP)
{
	std::array<vk::ClearValue, 2> clearValues{};
	clearValues[0] = vk::ClearColorValue(std::array<float, 4>({ 0.1f, 0.1f, 0.2f, 1.0f }));
	clearValues[1] = vk::ClearDepthStencilValue(1.0f, 0);
	...
	m_tArray->drawDynamic(cmd, m_linePipeline->getPipelineLayout(), duration, cameraPos, VP);
...
void RendererD::initSwapchainDependent(const Swapchain &swapchain)
{
...
void RendererD::freeSwapchainDependent()
{
