#!/usr/bin/python
# -*- coding: utf-8 -*-
# script for computing efficiency of a predicted sample
...
def printUsage():
	print "Usage: ./efficiency.py <file_name> <pid_labels>..."
...
for i in np.logspace(-1, 1, dtype='float64'):
	...
	for arg in args_list:
		...
		if all_chosen == 0:
			...
			scores_list[counter].append( all_true / all_particles )
			errors_list[counter].append( math.sqrt( math.pow(err_true, 2)/math.pow(all_particles, 2)
						+ math.pow(all_true, 2)*math.pow(err_all, 2)/math.pow(all_particles, 4) ) )
